...
| `AGENT_MODE` | `auto` | apply execution mode (auto/single/multi); the `--agents` flag overrides it |
| `EDIT_GUARD` | `on` | multi-mode out-of-bounds guard: blocks a role sub-agent from writing files outside its allow list |
