...
```bash
sdlc-review proposal <迭代目录>   # 运行 Gate 1：设计审查
sdlc-review code <迭代目录>       # 运行 Gate 2：代码审查
```
...
```bash
# 创建并行工作区
sdlc-worktree create <slug> <type>
# 示例
sdlc-worktree create user-login feature
sdlc-worktree create password-reset fix
...
# 移除已完成的工作区
sdlc-worktree remove <seq|slug>
sdlc-worktree remove --all-merged
...
IF 当前为 proposal 模式:
  ...
  LOG "   👉 审阅后请运行: sdlc-apply $ITER_DIR"
...
IF 当前为 apply 模式:
  ...
  IF NOT exists(STATUS_FILE):
    ERROR "未找到 status.json，请先运行 sdlc-proposal"
    ABORT
...
LOG "👉 浏览器验收: sdlc-qa   👉 提交: sdlc-accept"
```
