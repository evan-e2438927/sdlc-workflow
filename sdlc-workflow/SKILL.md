...
argument-hint: "init [配置] | proposal <需求> | apply [--review] [--e2e] <迭代目录> | review [proposal|code] <迭代目录> | accept <迭代目录> | doit [--review] [--e2e] <需求> | mini [--e2e] <小任务> | worktree <create|list|status|remove|gc>"
...
- `/sdlc-workflow apply [--review] [--e2e]`：人工审核通过后，从 proposal 产物继续执行开发到 PR；`--review` 触发 Codex 审查（Gate 1 + Gate 2）；`--e2e` 启用 Playwright E2E 浏览器测试（默认只跑 lint + unit）
...
- `/sdlc-workflow doit [--review] [--e2e]`：全自动模式（内部 proposal + apply 不停顿）；`--e2e` 同上
- `/sdlc-workflow mini [--e2e]`：小任务轻量流程；`--e2e` 同上
...
```bash
/sdlc-workflow apply [--review] [--e2e] <迭代目录>
# 示例
...
/sdlc-workflow apply --e2e docs/iterations/2026-04-16/001-user-login-feature/
```
...
⑨ test-pipeline（lint → unit；E2E 加 --e2e 才执行）
⑩ docs-updater
...
| ⑨ | test-pipeline | lint → unit（默认）；加 `--e2e` 才运行 Playwright E2E | apply/doit |
...
STAGE 3: npx playwright test     # Playwright E2E（仅 --e2e 时执行，默认跳过）
...
18. **Agent Team 并行**：在步骤 ⑦ test-generator、⑩ docs-updater 使用 Agent Team 并行；⑨ test-pipeline 默认串行（lint → unit），加 `--e2e` 时 Stage 2+3 可并行
