...
3. `unit-test` / `qa` Track 仅允许出现在 Phase 3；Phase 1/2 实现任务里随手写的单元测试**不计**为 unit-test Track（仍计入该实现任务自身）。`qa` Track 任务只描述浏览器验收场景，不含实现代码，apply 阶段跳过、由 `qa` 命令处理
...
- 参考：
  - references/05-design-reviewer.md（下一步，Gate 1）
