...
- 日期：2026-07-03
- 状态：**已修订为方案 B（见下方「修订」），已实现**
- 范围：让 qa 阶段产出「成功态验收截图」，并在 pr 阶段把它们作为证据嵌入 PR body

## 修订（2026-07-03 实测后）

真实项目（私有仓库）测试暴露方案 A 两处失效，改为 **方案 B**：

1. **QA 落盘位置不可控**：Playwright MCP 的 `take_screenshot` 默认存到工作目录根（cwd），不听 prose 指定路径 → 必须截图后**显式 `mv`**。
2. **私有仓库不渲染 + 不入库要求**：用户要求截图**保存在项目内但不入库**；且私有仓库里 `raw.githubusercontent.com` / `blob?raw=true` **不渲染**（GitHub camo 取不到私有内容），`gh` 也无法上传 PR 附件。方案 A（提交 evidence + raw URL）对私有仓库彻底失效。

**方案 B（现行）**：
- QA 截图 → `tests/reports/<slug>/screenshots/<Scenario-ID>.png`，**gitignore、仅本地、不入主干**（显式 `mv` 自 MCP 默认位置）。
- pr 阶段用 **git plumbing** 把截图写进**隔离的 `pr-assets` 分支**（不切分支、不碰工作区、不进 main/develop），PR 正文内联引用 `github.com/<repo>/raw/pr-assets/...`，并附**可点击兜底链接**（私有仓库若内联不渲染仍可一键查看）。
- 三诉求权衡：**私有 + 不入主干 + PR 带图**，代价是截图进了隔离 assets 分支（永不合并），且私有仓库内联渲染不保证（兜底链接可达）。

> 以下为初版（方案 A）设计记录，保留供追溯；实现以「修订」为准。
